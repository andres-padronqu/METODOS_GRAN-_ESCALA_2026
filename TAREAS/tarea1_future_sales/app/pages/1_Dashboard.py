...
col2.metric("Pronóstico", f"{total_prediction:,.0f}")
...
fig = px.line(
    ...
    title="Ventas reales vs pronóstico vs baseline naive",
)
...
category_error = df.assign(
    abs_error=(df["actual"] - df["prediction"]).abs()
).groupby("item_category_name", as_index=False)["abs_error"].mean()
